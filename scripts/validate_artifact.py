...
def validate_approved_profile(config: dict[str, Any]) -> None:
    """Enforce the approved organization inventory without personal profile data."""
    profile = config["profile"]
    if profile["username"] != "doka-labs" or profile["organization"] != "doka-labs":
        fail("organization identity is incorrect")

    if profile.get("maintainer_username") != "kdominic89":
        fail("approved maintainer is missing")

    if profile.get("maintainer_role") != "Administrator & Core Maintainer":
        fail("approved maintainer role is missing")

    domains = config.get("domains", [])
    if len(domains) != 1 or domains[0].get("id") != "doka-labs" or domains[0].get("kind") != "organization":
        fail("only the organization domain may be published")

    projects = {item["id"] for item in config.get("projects", [])}
    if projects != {"mysql", "safe-migrations", "relational-lab"}:
        fail("organization must contain its three approved projects")

    packages = {item["id"] for group in config.get("publications", []) for item in group.get("packages", [])}
    if packages != EXPECTED_PACKAGES:
        fail("organization must contain its six approved packages")

    if config.get("interests") or config.get("learning") or config.get("presentation", {}).get("hardware"):
        fail("personal context must not appear in the organization profile")

    collection = config["collection"]
    if collection.get("github_user") or collection.get("collect_contributions") or collection.get("collect_private_repository_count"):
        fail("organization profile must not collect personal account data")
...
def validate_readme_and_site(root: Path) -> None:
    ...
    for reference in (
        ...
        "assets/sourcefield.light.svg",
        "assets/sourcefield.static.svg",
        "https://doka-labs.github.io/.github/",
    ):
    ...
    organization_readme = (root / "profile/README.md").read_text(encoding="utf-8")
    for reference in (
        "https://raw.githubusercontent.com/doka-labs/.github/main/assets/sourcefield.dark.svg",
        "https://raw.githubusercontent.com/doka-labs/.github/main/assets/sourcefield.light.svg",
        "https://raw.githubusercontent.com/doka-labs/.github/main/assets/sourcefield.static.svg",
        "https://github.com/kdominic89",
        "Administrator & Core Maintainer",
        "https://doka-labs.github.io/.github/",
        "prefers-reduced-motion",
    ):
        if reference not in organization_readme:
            fail(f"organization README misses {reference}")
...
def validate_workflows(root: Path) -> None:
    ...
    if "PROFILE_TOKEN" in update or "private-counts" in update:
        fail("organization workflow must not collect private account data")
...
def main() -> int:
    ...
    parser.add_argument(
        "--approved-profile", action="store_true", help="Also enforce the approved organization profile facts."
    )
