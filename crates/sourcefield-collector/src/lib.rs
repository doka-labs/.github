...
impl Collector {
    ...
    pub async fn collect(
        ...
        let collect_user = !config.collection.github_user.is_empty();
...
mod tests {
    ...
    fn config() -> Config {
        ...
        config.collection.github_user = "owner".into();
    ...
    async fn organization_collection_uses_only_public_org_endpoints_even_with_user_flags() {
        ...
        config.collection.github_organizations = vec!["doka-labs".into()];
        ...
        assert!(requests[0].starts_with("GET /orgs/doka-labs HTTP/1.1"));
        assert!(requests[1].starts_with("GET /orgs/doka-labs/repos?type=public&sort=pushed"));
        assert!(
            requests
                .iter()
