...
/// Restrict observed source data to the approved collection scope before publication.
///
/// Organization profiles omit the user collection account. Their cached snapshots may still
/// contain old personal observations, so every raw and derived output must use this boundary.
/// Personal-profile callers retain their independently authorized user collection policy.
pub fn scope_snapshot(config: &Config, snapshot: &mut Snapshot) {
    if !config.collection.github_user.is_empty() {
        return;
    }

    let organizations = &config.collection.github_organizations;
    let configured = |owner: &str| {
        organizations
            .iter()
            .any(|org| org.eq_ignore_ascii_case(owner))
    };

    snapshot.user = Default::default();
    snapshot.contributions = None;
    snapshot.private_repository_count = None;
    snapshot
        .organizations
        .retain(|organization| configured(&organization.login));
    snapshot
        .repositories
        .retain(|repository| configured(&repository.owner));
    snapshot.sources.retain(|source| {
        if matches!(
            source.source.as_str(),
            "github:user" | "github:contributions" | "github:private-count"
        ) {
            return false;
        }
...
pub fn build_state(
    ...
    let mut snapshot = normalized_snapshot(snapshot);
    ...
    let state = ProfileState {
        ...
        profile: StateProfile {
            maintainer_username: config.profile.maintainer_username.clone(),
            maintainer_role: config.profile.maintainer_role.clone(),
