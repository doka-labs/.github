...
pub fn validate_config(config: &Config) -> Result<(), ValidationError> {
    ...
    account(&config.profile.maintainer_username)?;
    if config.profile.maintainer_role.trim().is_empty() {
...
pub fn validate_state(state: &ProfileState) -> Result<(), ValidationError> {
    ...
    account(&state.profile.maintainer_username)?;
    if state.profile.maintainer_role.trim().is_empty() {
        return Err(ValidationError::InvalidValue("maintainer role".into()));
    }
