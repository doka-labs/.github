...
pub fn render_svg(state: &ProfileState, theme: Theme, motion: bool) -> String {
    ...
    footer(&mut output, state, palette);
...
fn definitions(output: &mut String, p: Palette, motion: bool, state: &ProfileState) {
    ...
    let _ = write!(
        output,
        "<linearGradient id=\"organization-gradient\" x1=\"0\" y1=\"0\" x2=\"1\" y2=\"1\">\
    ...
    if motion {
        ...
        for index in 0..count {
            ...
            let _ = write!(
                output,
                ".signal[data-satellite=\"nuget-package\"][data-signal-delay=\"{delay:.2}\"]\
...
fn field(output: &mut String, state: &ProfileState, nodes: &BTreeMap<&str, &Node>, p: Palette) {
    ...
    let sy = state.canvas.height as f32 / 1320.0;
        ...
        .filter(|node| node.kind == NodeKind::Domain && node.show_in_readme)
    {
        ...
        let center_y = domain.y + 30.0 * sy;

        let _ = write!(
            output,
            r#"<ellipse cx="{}" cy="{center_y}" rx="{}" ry="{}" fill="url(#amber)"/>"#,
            domain.x,
            650.0 * sx,
            315.0 * sy
        );
        ...
        if state.canvas.show_activity_orbit {
            let _ = write!(
                ...
                "<ellipse cx=\"{}\" cy=\"{center_y}\" rx=\"{}\" ry=\"{}\" fill=\"none\" \
                 stroke=\"{color}\" stroke-opacity=\".16\" class=\"flow\"/>",
                domain.x,
                510.0 * sx,
                250.0 * sy
            );
    ...
    for edge in edges {
        ...
        let Some(d) = connection_path(from, to, state) else {
            continue;
        };
        ...
        path(output, &d, node_color(from, p), ".28", "");
        path(output, &d, node_color(from, p), ".62", "flow");
        output.push_str("</g>");
    }
...
/// Shape ownership curves by relative position, with radial tangents at both rings.
fn connection_path(from: &Node, to: &Node, state: &ProfileState) -> Option<String> {
    let sx = state.canvas.width as f64 / 1800.0;
    let sy = state.canvas.height as f64 / 1320.0;
    let a = Point(from.x as f64 / sx, from.y as f64 / sy);
    let b = Point(to.x as f64 / sx, to.y as f64 / sy);
    let dx = b.0 - a.0;

    // Mirrored upper handles retain the approved sweep without binding geometry to project IDs.
    let controls = if dx.abs() > (b.1 - a.1).abs() {
        let side = dx.signum();
        let approach = if side > 0.0 { 201.0 } else { 205.0 };

        [
            Point(a.0 + side * 143.134, a.1 - 143.134),
            Point(b.0 - side * approach, b.1),
        ]
    } else {
        [Point(a.0 + 140.0, a.1 + 140.0), Point(b.0, b.1 - 140.0)]
    };
...
fn publications(
    ...
    path(output, "M64 882H1736", p.grid, ".5", "");
    text(
        output,
        (66.0, 922.0),
...
fn footer(output: &mut String, state: &ProfileState, p: Palette) {
    path(output, "M64 1255H1736", p.grid, ".45", "");
    ...
    let maintainer_url = format!("https://github.com/{}", state.profile.maintainer_username);
    ...
    text(
        ...
        &format!(
            "{} / {}",
            state.profile.maintainer_username, state.profile.maintainer_role
        ),
        12,
        p.muted,
        "end",
...
mod tests {
    ...
    fn approved_inventory_and_copy_are_rendered() {
        ...
        assert!(svg.contains("Libraries and tools for .NET and relational databases."));
        assert!(svg.contains("https://github.com/kdominic89"));
        assert!(svg.contains("Administrator &amp; Core Maintainer"));
        assert!(!svg.contains("data-ring=\"personal"));
        assert!(!svg.contains("AT HOME"));
        assert!(!svg.contains("SOURCEFIELD / 03"));
    }

    #[test]
    fn organization_connections_use_curves_and_actual_ring_boundaries() {
        let (_, state) = fixture();
        let hub = state
            .nodes
            .iter()
        ...
        let mysql = state
            .nodes
            .iter()
            .find(|node| node.id == "project:mysql")
            .unwrap();

        let curve = connection_path(hub, mysql, &state).unwrap();
        let values = curve
            .split(['M', 'C', ' '])
        ...
        let endpoint_distance =
            (values[6] - f64::from(mysql.x)).hypot(values[7] - f64::from(mysql.y));
        let mut overlapping = mysql.clone();
        overlapping.x = hub.x;
        overlapping.y = hub.y;

        assert!((endpoint_distance - f64::from(mysql.radius)).abs() < 1e-5);
        assert_ne!(values[1], values[3]);
        assert_ne!(values[3], values[5]);
        assert!(connection_path(hub, &overlapping, &state).is_none());
    }
    ...
    fn optional_content_and_canvas_dimensions_are_consumed() {
        ...
        assert!(!svg.contains("stroke-opacity=\".16\" class=\"flow\""));
    }
