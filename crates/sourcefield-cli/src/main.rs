...
use sourcefield_core::{
    DataStatus, ProfileState, Snapshot, SnapshotMode, build_state, load_config, scope_snapshot,
    validate_config, validate_state,
};
...
async fn generate(options: GenerateOptions<'_>) -> Result<()> {
    ...
    // Apply the same scope to reproducibility inputs and derived artifacts; sanitizing only
    // the graph would still publish stale personal observations in source-snapshot.json.
    scope_snapshot(&config, &mut snapshot);
...
mod tests {
    ...
    #[tokio::test]
    async fn organization_generation_removes_personal_observations_from_every_output() {
        let directory = TemporaryDirectory::new();
        let root = Path::new(env!("CARGO_MANIFEST_DIR")).join("../..");
        let config = root.join("config/profile.toml");
        let mut snapshot: Snapshot = read_json(&root.join("config/offline-snapshot.json")).unwrap();
        snapshot.user.login = "forbidden-personal-observation".into();
        snapshot.user.followers = 456789;
        snapshot.private_repository_count = Some(456789);
        snapshot.contributions = Some(sourcefield_core::ContributionSnapshot {
            total: 456789,
            ..Default::default()
        });
        snapshot
            .repositories
            .push(sourcefield_core::RepositorySnapshot {
                owner: "forbidden-personal-observation".into(),
                name: "private-project".into(),
                full_name: "forbidden-personal-observation/private-project".into(),
                ..Default::default()
            });
        snapshot.sources.push(sourcefield_core::SourceStatus {
            source: "github:user".into(),
            status: DataStatus::Live,
        });
        let fallback = directory.0.join("fallback.json");
        fs::write(&fallback, serde_json::to_vec(&snapshot).unwrap()).unwrap();
        ...
        let mut directories = vec![assets.clone(), docs];
        while let Some(directory) = directories.pop() {
            for entry in fs::read_dir(directory).unwrap() {
                ...
                assert!(
                    !content.contains("forbidden-personal-observation"),
                    "{}",
                    path.display()
                );
                assert!(!content.contains("456789"), "{}", path.display());
                assert!(!content.contains("github:user"), "{}", path.display());
            }
        }

        let published: Snapshot = read_json(&assets.join("source-snapshot.json")).unwrap();
        assert!(published.user.login.is_empty());
        assert!(published.contributions.is_none());
        assert!(published.private_repository_count.is_none());
        assert_eq!(published.packages.len(), 6);
        assert_eq!(published.repositories.len(), 2);
    }
    ...
    #[test]
    fn organization_state_rejects_private_counts_even_with_explicit_opt_in() {
        let root = Path::new(env!("CARGO_MANIFEST_DIR")).join("../..");
        ...
        assert_eq!(enabled.stats.private_repository_count, None);
