...
class OrganizationContractTests(unittest.TestCase):
    """Keep the organization artifact separate from the maintainer's personal profile."""

    def test_approved_organization_and_maintainer_are_accepted(self):
        root = Path(__file__).resolve().parents[1]
        config = validator.validate_config(root)

        validator.validate_approved_profile(config)
        validator.validate_readme_and_site(root)

    def test_personal_collection_and_inventory_are_rejected(self):
        root = Path(__file__).resolve().parents[1]
        original = validator.validate_config(root)
        for mutation in ("account", "project", "hardware", "maintainer"):
            with self.subTest(mutation=mutation):
                config = json.loads(json.dumps(original))
                if mutation == "account":
                    config["collection"]["github_user"] = "kdominic89"
                elif mutation == "project":
                    config["projects"].append({"id": "personal-project"})
                elif mutation == "hardware":
                    config.setdefault("presentation", {})["hardware"] = [{"label": "Private hardware"}]
                else:
                    config["profile"]["maintainer_username"] = ""
