...
class PresentationTests(unittest.TestCase):
    ...
    def test_connection_boundaries_and_curvature(self):
        for flavor in ('dark', 'light', 'static'):
            ...
            for group, source, target in pairs:
                ...
                chord = (points[3][0] - points[0][0], points[3][1] - points[0][1])
                handle = (points[1][0] - points[0][0], points[1][1] - points[0][1])
                self.assertGreater(abs(chord[0] * handle[1] - chord[1] * handle[0]), 1)
    ...
    def test_independent_rings_and_signal_inventory(self):
        ...
        for kind, count in (('nuget-package', 6),):
        ...
        self.assertFalse(any(e.get('data-domain') == 'personal' for e in svg.iter()))
        links = [e.get('href') for e in svg.iter(f'{NS}a')]
        self.assertIn('https://github.com/kdominic89', links)
