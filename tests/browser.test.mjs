...
test('a tap updates its coordinates without any pointermove', async () => {
    ...
    harness.evaluate(`app.layer = 'systems'; app.size = {width:1800,height:1320,fit:1};
...
test('snapshot validation rejects corrupt geometry before active-state mutation', async () => {
    ...
    harness.evaluate(`globalThis.validSnapshot = {
        ...
        canvas: {width:1800,height:1320}, profile: {}, stats: {}, mode:'preview',
    ...
    assert.throws(() => harness.evaluate('validateState({...validSnapshot, canvas:{width:NaN,height:1320}})'));
...
test('organization counters preserve zero and do not expose personal metrics', async () => {
    const harness = await browserHarness();
    harness.evaluate("app.state = {stats:{organization_public_repositories:0,package_count:6,private_repository_count:99,followers:99},mode:'live'}; renderMetrics()");

    const metrics = harness.elements.get('#metrics').children;

    assert.equal(metrics.length, 2);
    assert.equal(metrics[0].children[0], 'REPOSITORIES ');
    assert.equal(metrics[0].children[1].textContent, '0');
    assert.equal(metrics[1].children[1].textContent, '6');

    harness.evaluate('delete app.state.stats.organization_public_repositories; renderMetrics()');

    assert.equal(harness.elements.get('#metrics').children[0].children[1].textContent, 'unavailable');
});

test('maintainer uses a GitHub link and malformed account metadata cannot navigate', async () => {
    const harness = await browserHarness();
    const link = harness.elements.get('#maintainer-link') ?? harness.context.document.querySelector('#maintainer-link');
    link.removeAttribute = name => { delete link[name]; };
    harness.evaluate("app.state = {profile:{maintainer_username:'kdominic89',maintainer_role:'Administrator & Core Maintainer'}}; renderMaintainer()");

    assert.equal(link.href, 'https://github.com/kdominic89');
    assert.equal(link.textContent, 'kdominic89 / Administrator & Core Maintainer');
    assert.equal(link.hidden, false);

    for (const username of ['../evil', 'user@evil.example', 'javascript:alert(1)', '', 'bad--name']) {
        harness.context.invalidUsername = username;
        harness.evaluate('app.state.profile.maintainer_username = invalidUsername; renderMaintainer()');
...
test('profile animation timing preserves orbit speed and safe signal phases', async () => {
    const harness = await browserHarness();
    const timings = [];
    const fixtures = [
    ...
    assert.deepEqual(timings.slice(0, 5), [
        { duration: 58000 }, { duration: 58000 }, { duration: 32000 },
