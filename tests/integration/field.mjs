...
async function checkMotion(page) {
    return page.evaluate(() => {
        ...
        try {
            ...
            if (directions.length !== 14 || signals.length !== 6) {
            ...
            if (scan) {
                throw new Error('Personal radar scan must not appear in the organization profile');
            }

            return {
                directions,
                timing,
                personalScanAbsent: true,
...
/** Check browser path geometry, radial tangency, distinct organization ports, and package trunks. */
async function checkConnections(page) {
    const results = await page.evaluate(() => {
        ...
        let minAlignment = 1;
        const seen = new Set();
        const organizationAngles = [];

        for (const path of svg.querySelectorAll('[data-edge-id] path')) {
            ...
            const controlPoints = path.getAttribute('d').match(/-?\d+(?:\.\d+)?/g).map(Number);

            if (parent.dataset.to !== 'project:relational-lab') {
                const [sx, sy, cx, cy, , , ex, ey] = controlPoints;
                const deviation = Math.abs((cx - sx) * (ey - sy) - (cy - sy) * (ex - sx));

                if (!path.getAttribute('d').includes('C') || deviation < 100) {
                    throw new Error(`Organization connection lost its curve: ${id}`);
                }
            }
            ...
            if (parent.dataset.from === 'domain:doka-labs') {
                organizationAngles.push(Math.atan2(start.y - source.y, start.x - source.x) * 180 / Math.PI);
            }
        ...
        organizationAngles.sort((left, right) => left - right);
        const organizationGaps = organizationAngles.map((angle, index) =>
            (organizationAngles[(index + 1) % organizationAngles.length] - angle + 360) % 360);

        if (organizationAngles.length !== 3 || organizationGaps.some(gap => gap < 20)) {
            throw new Error('Organization connections must leave from three distinct ports');
        }
        ...
        return {
            ...
            organizationPortAngles: organizationAngles,
            organizationPortGaps: organizationGaps,
            maxEndpointError,
    ...
    assert.equal(results.curveCount, 3);
