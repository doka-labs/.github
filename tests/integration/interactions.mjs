...
export async function runInteractionChecks({ browser, base, output }) {
    ...
    try {
        ...
        assert.equal(await page.locator('#profile-view [data-node-id]').count(), 10);
        assert.equal(await page.locator('#domain-button, #profile-context').count(), 0);
        assert.equal(await page.locator('#profile-view [data-domain="personal"]').count(), 0);
        assert.equal(await page.locator('#maintainer-link').getAttribute('href'), 'https://github.com/kdominic89');
        assert.equal(await page.locator('#maintainer-link').textContent(),
            'kdominic89 / Administrator & Core Maintainer');
        results.push({
        ...
        const projectButton = page.locator('#node-list button').filter({ hasText: 'Doka.EntityFrameworkCore.MySql' }).first();
