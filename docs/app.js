...
/** Link only a validated GitHub account, keeping malformed metadata out of navigation. */
function renderMaintainer() {
    const link = document.querySelector('#maintainer-link');
    const { maintainer_username: username, maintainer_role: role } = app.state.profile ?? {};
    ...
    link.textContent = role ? `${username} / ${role}` : username;
}
...
function renderMetrics() {
    ...
    const signals = [
        ['REPOSITORIES', stats.organization_public_repositories],
...
/** Resolve layer visibility without changing canonical state. */
...
function nodeColor(node, palette) {
    ...
    if (node.kind === 'component' || node.kind === 'technology') return palette.accent2;
...
function edgeColor(edge, palette) {
    ...
    return edge.kind === 'implemented-with' ? palette.accent2 : palette.quiet;
}
...
function drawOverlay(visual, palette, time) {
    ...
    // A subtle contour preserves the organization anchor as exploratory nodes move.
...
function readPalette() {
    ...
    return {
        ...
        accent: value('--accent'), accent2: value('--accent-2'),
